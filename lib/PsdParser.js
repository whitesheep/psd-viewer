...
export default class PsdParser {
  ...
  constructor(uri, cacheDir) {
    ...
    this.imageUri = `${cacheDir}/${this._imageHash}.png`;
    this.jsonUri = `${cacheDir}/${this._imageHash}.json`;
  }
  ...
  async parse() {
    ...
    psd.image.width = () => 100;
    psd.image.height = () => 100;
    return psd.image.saveAsPng(this.imageUri)
      .then(() => extractedData)
  ...
  parseObject(data) {
    ...
    if (data.objectEffects) {
      ...
      layerStyles.textShadow = data.node.text
        ? layerStyles.dropShadow
        : undefined;
    ...
    layerStyles.backgroundColor = data.solidColor
      ? `rgb(${data.solidColor.join(', ')})`
      : undefined;
